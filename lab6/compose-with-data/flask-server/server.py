...
""" Startup Information """
# Open the JSON file
# WARNING: IN DEBUG THIS RUNS TWO TIMES
if students_collection.count_documents({}) == 0 and professors_collection.count_documents({}) == 0:
    with open('startup-info/data.json') as f:
        users = json.load(f)
        
        # Parse all the users in the JSON file
        for user in users:
            if user["is_teacher"]:  # If user["is_teacher"] == True:
                professor = {'name': user["name"], 'surname': user["surname"], 'subject': user["subject"]}
                professors_collection.insert_one(professor) # Add this professor
            else:
                student = {'name': user["name"], 'surname': user["surname"]}
                students_collection.insert_one(student)     # Add this student
